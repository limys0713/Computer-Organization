...
////////////////////////////////////////////////
  ...
  if(idx % 3 == 0){//fifo
    ...
    for(size_t i = 0; i < ways; i++){
      if(lru[idx][i] == 0){  //if that block is least recently used the indicate sequence number is 0
        way = i;
        break;
      }
    }

    lru[idx][way] = ways - 1;     //recently used, so set the sequence to ways - 1
    for (size_t j = 0; j < ways; j++){
      if(j != way){
        lru[idx][j]--;    //others sequence -1
      }
    }
  }
  else{ //lfu
    for(size_t i = 0; i < ways; i++){
      if(lfu[idx][i] < min_lfu && lfu[idx][i] >= 0){
        min_lfu = lfu[idx][i];
        way = i;
      }
    }
