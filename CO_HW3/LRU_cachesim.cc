...
//////////////////////////////////////
  for(size_t i = 0; i < sets; i++){
    for(size_t j = 0; j < ways; j++){
      record_LRU[i][j] = j; //initialize  LRU sequence number array (0 1 2 3...)
    }
}
//////////////////////////////////////
}
...
uint64_t* cache_sim_t::check_tag(uint64_t addr)
{
  ...
  for (size_t i = 0; i < ways; i++)
    if (tag == (tags[idx*ways + i] & ~DIRTY)){
      ...
      //////////////////////////////////////////////////////////////
      for (size_t j = 0; j < ways; j++){
        if(record_LRU[idx][j] > record_LRU[idx][i]){ //sequence number that are higher must decrement by 1 and set the sequence number of the hit block to ways-1
          record_LRU[idx][j]--;    
        }
      }
      record_LRU[idx][i] = ways - 1; 
      //////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////
  ...
  for(size_t i = 0; i < ways; i++){
    if(record_LRU[idx][i] == 0){  //if that block is least recently used the indicate sequence number is 0
     way = i;
     break;
    }
  }

  record_LRU[idx][way] = ways - 1;     //recently used, so set the sequence to ways - 1
  for (size_t j = 0; j < ways; j++){
    if(j != way){
      record_LRU[idx][j]--;    //others sequence -1
    }
  }
  ///////////////////////////////////////
